...
ZH_BUNDLE = TextBundle(
    ...
    llm=LLMBundle(
        ...
        subagent_addenda={
            "open-coding": "\n\n读取 corpus/raw.jsonl 与 coding/ 文件，必要时用 write_file 写入 coding/{id}.json。",
            "construct-induction": "\n\n从 coding/ 中读取 partition.codebook_text_ids 对应 seed pool 文件，归纳后写入 codebook/dimensions.json。",
            "axial-coding": "\n\n读取 codebook/dimensions.json 与 update-pool 批次对应 coding 文件，输出完整更新版 dimensions。",
        },
        schema_descriptions={
            ...
            "open_coding_content_with_labels": "带有标记的原始语料文本，只在被提取片段外包裹 <p>...</p> 标签，不改写原句或使用其他标签。",
            ...
            "semantic_match_matched_human_dimensions": "匹配到的 human 维度名称列表，可为空列表。",
    ...
    cli=CLIBundle(
        ...
        banner_hint="/status /tree /export /eval:open /clear /help · Esc 中断 · exit 退出",
        help_text="\n".join(
            [
                "Slash 命令:",
...
EN_BUNDLE = TextBundle(
    ...
    llm=LLMBundle(
        ...
        subagent_addenda={
            "open-coding": "\n\nRead corpus/raw.jsonl and coding/ files, and use write_file to write coding/{id}.json when needed.",
            "construct-induction": "\n\nRead coding/ files for partition.codebook_text_ids in the seed pool, then write the synthesized codebook to codebook/dimensions.json.",
            "axial-coding": "\n\nRead codebook/dimensions.json and coding files for the update-pool batch, then output the full updated dimensions array.",
        },
        schema_descriptions={
            "open_coding_item_name": "A concise Chinese summary of the extracted fragment.",
            "open_coding_item_evidence": "Exact or approximate source evidence from the original review.",
            "open_coding_item_normalized_label": "The primary Chinese dimension for this item.",
            "open_coding_item_reason": "One brief Chinese sentence explaining why the evidence supports the dimension.",
            "open_coding_content_with_labels": "The original corpus text with markup; wrap only extracted fragments in <p>...</p>; do not rewrite the sentence or use other tags.",
            "induction_dimension_name": "Dimension name.",
            "induction_dimension_items": "Chinese item labels in this dimension; each must come from items_details.label or the original items_pool labels.",
            "induction_dimension_definition": "One concise Chinese sentence defining the boundary of this dimension.",
            "axial_coding_dimension_name": "Dimension name.",
            "axial_coding_dimension_items": "Chinese item labels in this dimension; each must be an existing codebook item or an original label from the current batch input.",
            "axial_coding_dimension_definition": "One concise Chinese sentence defining the boundary of this dimension.",
            "semantic_match_thought": "Optional brief rationale.",
            "semantic_match_matched_human_dimensions": "List of matched human dimension names; may be empty.",
    ...
    cli=CLIBundle(
        ...
        prompts_dir_help="Prompt set name or path. Defaults to baseline for zh and baseline_en for en.",
        language_help="Language for prompts, reports, and code-side text (default: zh or en).",
        banner_hint="/status /tree /export /eval:open /clear /help · Esc interrupts · exit quits",
        help_text="\n".join(
            [
                "Slash commands:",
